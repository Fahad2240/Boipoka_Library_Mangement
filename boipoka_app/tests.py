...
#         self.deletenotification_url = reverse('boipoka_app:deletenotification', args=[self.notification.pk])
    ...
    #     # Compare the notifications queryset as a list
    ...
    #     self.assertTemplateUsed(response, 'boipoka_app/notification.html')
    #     # Compare the notifications queryset as a list
    #     self.assertEqual(list(response.context['notifications']), [unread_notification])
    ...
    #     self.assertTemplateUsed(response, 'boipoka_app/notification.html')
    #     # Compare the notifications queryset as a list
    #     self.assertEqual(list(response.context['notifications']), [read_notification])
    ...
    #     self.assertEqual(response.status_code, 200)
    #     self.assertTemplateUsed(response, 'boipoka_app/notification.html')
    #     # Compare the notifications queryset as a list, unordered
...
class RegisterViewTest(TestCase):
    def setUp(self):
        ...
        self.client = Client()
        # Set up URL for registration
        self.url = reverse('boipoka_app:register')
        # Set up test data for the form
        self.user_data = {
            'username': 'testuser',
    ...
    def test_get_register_page(self):
        """Test accessing the register page with a GET request"""
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_already_logged_in(self):
        """Test that a logged-in user is redirected when accessing the register page"""
        # Create a user and log them in
        user = User.objects.create_user(username='loggedinuser', password='password123')
        self.client.login(username='loggedinuser', password='password123')

        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 302)  # Should be redirected
        self.assertRedirects(response, '/')  # Redirects to homepage or preferred page when logged in
