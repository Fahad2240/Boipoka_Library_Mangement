...
The Notifications Feature will allow users to access notifications for every significant event by clicking the notifications button, which will display these alerts in chronological order, with the most recent events appearing first. An unread notifications counter will be shown in the top right corner of the button, indicating the number of notifications that have yet to be acknowledged. When a user clicks on a notification, the counter will automatically decrease, and that particular notification will no longer be clickable. Additionally, users will have the option to delete each notification, whether it is read or unread. Notifications will be managed through the Notifications Model, which includes fields for the subscriber, message, timestamp, and other relevant details. Each time a significant event occurs in a specific view, a new notification object will be created, with the is_read field set to false by default. Upon clicking a notification, the is_read status will be updated to true, reflecting the change in both the notification list and the unread notifications counter.
